...
function collectDrinkSalesStats() {
  ...
  function addTo(map, key, id, name, qty, revenue) {
    if (!map.has(key)) map.set(key, new Map());
    const inner = map.get(key);
    if (!inner.has(id)) inner.set(id, { name, qty: 0, revenue: 0 });
    ...
    for (const b of allBills) {
      for (const r of b.rounds || []) {
        ...
        for (const i of r.items || []) {
          ...
          if (!qty) continue;
          addTo(byDay, dayKey, i.id, i.name, qty, revenue);
          addTo(byMonth, monthKey, i.id, i.name, qty, revenue);
          addTo(byYear, yearKey, i.id, i.name, qty, revenue);
        }
      }
    }
  }
...
function renderBestSellers() {
  ...
  for (const key of keys) {
    ...
    const groupBtn = el(
      "button",
      "collapse-toggle",
      `${expanded ? "▾" : "▸"} ${label} — ${totalQty} ชิ้น • ฿${money(totalRevenue)}`
    );
