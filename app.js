...
function renderHome() {
  ...
  const voiceBtn = el(
    "button",
    "btn-secondary",
    VOICE_ORDER_LISTENING ? "🎤 กำลังฟัง... (พูดตอนนี้ได้เลย)" : "🎤 สั่งเครื่องดื่มด้วยเสียง"
  );
  voiceBtn.style.cssText = "width:100%;margin-bottom:10px;" + (VOICE_ORDER_LISTENING ? "background:var(--yellow-dark);" : "");
  voiceBtn.disabled = VOICE_ORDER_LISTENING;
  voiceBtn.onclick = startVoiceOrder;
  APP.appendChild(voiceBtn);
  if (VOICE_ORDER_LAST && VOICE_ORDER_LAST.raw) {
    const lastNote = el(
      "div",
      "round-meta",
    ...
    for (const r of rounds) {
      ...
      if (RETURN_ITEM_SHOW === r.id && returnableItems.length) {
        ...
        returnPanel.appendChild(
          el("div", "round-meta", "เลือกจำนวนที่ลูกค้าคืน (ไม่รับแล้ว) สต็อกจะถูกคืนกลับให้อัตโนมัติ")
        );
        for (const i of returnableItems) {
          const row = el("div", "round-item");
          const rTop = el("div", "round-top");
          rTop.appendChild(el("span", null, `${i.name} (มี ${i.qty}${i.free ? " ฟรี" : ""})`));
          row.appendChild(rTop);
        ...
        confirmReturnBtn.onclick = async () => {
          const toReturn = Object.entries(RETURN_ITEM_DRAFT).filter(([, qty]) => Number(qty) > 0);
          if (!toReturn.length) {
            toast("ยังไม่ได้เลือกจำนวนที่จะคืนเลย", true);
            return;
          }
          SAVING = true;
          render();
          try {
            for (const [itemId, qty] of toReturn) {
              STATE = await apiReturnItem(locationId, r.id, itemId, Number(qty));
            }
            RETURN_ITEM_SHOW = null;
            RETURN_ITEM_DRAFT = {};
